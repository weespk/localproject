...
public class OpenRtbService {
			...
			String[][] info = new String[][] 
					{
						...
						{ "OpenRtbBid", "OpenRTBBid_Mixer", "10000" },
						{ "OpenRtbBid", "OpenRTBBid_Igaworks", "10000" },
						...
						{ "OpenRtbClickView", "OpenRTBClick_Igaworks", "100000" },
						{ "OpenRtbClickView", "OpenRTBView_Igaworks", "100000" },
						{ "OpenRtbClickView", "OpenRTBClick_Mixer", "100000" },
						{ "OpenRtbClickView", "OpenRTBView_Mixer", "100000" },
		
						//point
						{ "OpenRtbPoint", "OpenRTBPoint_Adfit", "100000" },
						{ "OpenRtbPoint", "OpenRTBPoint_Google", "100000" },
						{ "OpenRtbPoint", "OpenRTBPoint_Mixer", "100000" },
						{ "OpenRtbPoint", "OpenRTBPoint_Igaworks", "100000" },
						
						//conv
						{ "OpenRtbConv", "OpenRTBConversion_Adfit", "10000" },
						{ "OpenRtbConv", "OpenRTBConversion_Google", "10000" },
						{ "OpenRtbConv", "OpenRTBConversion_Mixer", "10000" },
						{ "OpenRtbConv", "OpenRTBConversion_Igaworks", "10000" }
					};
			boolean chk = false;
	...
	private ArrayList chkingTopicLag(String groupName, String topic, long maxLagCnt) {
		...
		try {
//			String kafkaGroupSummeryUrl = "http://192.168.2.77:9000/api/status/kafka/"+ groupName +"/KF/groupSummary";
	...
	public void chkingBatchRuningTime() {
		log.info("START OpenRtbService chkingBatchRuningTime");
		try {
			StringBuffer sbf = new StringBuffer("OpenRtb - BATCH DELAY\n");
			
			boolean chk = false;
			
			String []batchRunningTime = {	"http://10.251.0.215:8001/cron/openRtbBatchCheck.txt"};
			...
			for( String row : batchRunningTime ) {
				...
				for( Map i : list ) {
					...
					if( !"OK".equals(i.get("CHKING_STATS").toString()) ) {
						sbf.append(String.format("%s  마지막 구동시간 : %s \n",i.get("EXE_TYP_TIT"), i.get("LAST_EXE_TIME").toString()));
						chk = true;
					}
				}
	...
	public void chkingConsumerQueue() {
		...
		try {
			...
			String []kafkaGroupSummeryUrl = 
				{
//					"http://10.251.0.85:8000/bid/cron/openrtbConsumerQueueSize.txt",
//					"http://10.251.0.85:8000/clickview/cron/openrtbConsumerQueueSize.txt",
//					"http://10.251.0.85:8000/conv/cron/openrtbConsumerQueueSize.txt",
